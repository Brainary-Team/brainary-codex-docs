...
- 客户端对服务端的**反向请求一律自动批准**（`runner/rpc_client.py` 里写死的），配合 profile 里的"从不询问"审批策略，整条链路上**不存在任何人工审批点**
...
> [!NOTE]
> **这是实现的现状，不是设计上的取舍。** **issue #21** 正在推进"让程序能问人"，方案已定、尚未落地：由运行器经 MCP elicitation 承载，脚本侧仍是一次普通的工具调用。
