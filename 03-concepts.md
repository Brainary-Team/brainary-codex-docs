...
本篇覆盖编写 PoA 程序所需的全部背景，共四节。把程序打成包分发属于另一个阶段，见《08-packaging.md》文档。
...
graph TD
    ...
    PROG -->|"直接调用，当场拿到结果"| TOOLS["<b>PoA 工具面</b><br/>全局对象 tools 上的一批函数<br/>exec_command —— 跑 shell（是个真 shell）<br/>apply_patch —— 改文件<br/>view_image —— 看图"]
    PROG -->|"派出去，先拿到一个 handle"| AGENT["<b>子 agent</b><br/>一个独立的 AI 会话，有自己的上下文<br/>接收一段任务文字<br/>自己决定读什么、怎么读<br/>干完交回一段回答"]
    AGENT -->|"在自己的 thread 上取得"| CHILD_TOOLS["<b>子 agent 工具面</b><br/>不保证与 PoA 工具面相同<br/>不含包自带 MCP"]
    PROG -.->|"何时去等由程序决定"| AGENT

    style PROG fill:#dae8fc,stroke:#6c8ebf,stroke-width:2px
    style TOOLS fill:#f5f5f5,stroke:#666666
    style CHILD_TOOLS fill:#f5f5f5,stroke:#666666
...
- **子 agent**：一个独立的 AI 会话，有自己的上下文和工具。它接收一段任务文字，自己决定读哪些文件、怎么读，最后交回一段回答。它是异步的，派出去先拿到一个 handle，何时去等由程序决定。它在自己的 thread 上取得工具面，不保证与 root 完全相同；尤其是 `.poa` 包自带的 MCP 只注册到 root thread，子 agent 拿不到。
...
| prelude 提供的 | `AGENT_BACKEND`、`requireAgents`、`mapLimit`、`spawnAgent`、`spawnMany`、`collectAll`、`runBatch`、`sendAndWait`、`closeAll`、`parseJsonReply`、`shellLines`、`SAFE_NAME`、`hasTool`、`callTool`、`shapeOf`、`timed` | 一层普通 JS 封装，全文在 [《07 API 参考》§2.4](07-api-reference.md#24-prelude-全文) |

包的 `entry` 是**原样提交**的，前面不会被拼入任何内容。要用第二类那 16 个名字，就把
[《07 API 参考》§2.4](07-api-reference.md#24-prelude-全文) 整段抄进 `main.js`，抄在首行 pragma 之后——`// @exec:` 只在第 1 行生效，且不在第 1 行时不报错。另一种做法是只用内置那一档，《05-writing.md》§12.2 的范例即如此。
...
这一层把差异全部盖住，所以同一段程序在两代后端上写法完全一样。
...
JS 本身没有网络和文件系统，但 `tools.exec_command` 提供了一个真 shell。沙箱管的是 JS 引擎，不是能力边界；能力边界仍然是 codex 原本那套审批与沙箱策略。
...
大部分差异被 prelude 那一层盖住了；仍会影响程序结构的项目只在这里给出索引：

| 差异 | v1 | v2 |
| --- | --- | --- |
| 生命周期与批量边界 | 完成后仍占名额，必须显式关闭 | 完成后可能自动驱逐；两代批量写法见《05-writing.md》§6.3 |
| 递归深度限制 | 有，且拦两道，子 agent 天然派不出孙 agent | 没有，无条件放行。限制只能在 JS 里自行实现 |

批量写法见《05-writing.md》§6.3；递归风险见《05-writing.md》§6.4。
