...
> 用 `run.sh` 跑时 runner 会自动把它提回第 1 行，但**自研客户端拼源码时没有任何环节会代劳**——
> prelude 有 174 行，直接拼在前面就会把它挤到第 175 行。不报错，只是沿用 10 秒默认值，
...
写第二个程序时会立刻撞上这个问题：**PoA 没有启动参数。** 提交接口只收线程 id 和源码字符串两个字段，没有地方放参数。

于是实际可用的只有三条路：

| 想传什么 | 怎么传 |
| --- | --- |
| 少量常量（规模、并发度、目标名） | **直接写死在源码顶部**，或由自研客户端做字符串拼接后再提交 |
