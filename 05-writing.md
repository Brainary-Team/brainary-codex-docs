...
`Promise.race` 拿不到最先完成的那个，程序侧的等待调用是串行的（机制见工作原理 §4）。
...
上面整篇讲的都是 `demos/*.js` 那种写法：散装一个文件，靠 runner 拼 prelude。要把它变成一个能交给别人跑的包，有五处必须动，其中前两处不动就是直接报错。
...
**② 工具名从 `ALL_TOOLS` 里找，不能写死。** 尤其是包自带的那些，`mcp__<server>__<tool>` 的前缀会被清洗、重名时加哈希后缀。写法与理由见 API 参考 §3.1。
