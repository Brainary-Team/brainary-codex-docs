...
有几节是**当前边界的产物**，不是设计意图。边界一解除就该回来改：

| 如果这件事落地 | 本篇哪里失效 |
| --- | --- |
| 补上程序侧的续跑手段 | §2 关于 `yield_control` 的结论、§9 的两档 `yield_time_ms` |
...
> **已经过期过一次**：本篇原来说"提交接口只收线程 id 和源码字符串"，2026-08 改成收一个包。这类改动不会有人来通知你，靠的就是这张表。
