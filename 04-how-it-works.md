...
前两节是概览 §3 的展开版；已经看过那一节的，可以从 [§3 `tools.foo()` 实际发生了什么](#3-toolsfoo-实际发生了什么)直接读起。
...
**② 模型侧有的能力，程序侧不一定有。** 因为这个入口只开了"提交"这一件事。最典型的是续跑：模型可以让一个中途让出的 cell 接着跑，程序没有对应的手段。详见边界与限制。
...
由此带来的影响：`ALL_TOOLS` 能表明它在，但表明不了怎么调。参数字段只能查文档，这正是 API 参考 存在的理由。
...
这是当前一个真实的能力缺口。API 参考 是它的离线替代品，但那是查表，不是运行时能力。
