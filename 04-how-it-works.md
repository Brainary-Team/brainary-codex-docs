...
> 顺带：不标注还有另一个后果——[默认审批模式下会触发 elicitation，无人值守时就是挂死](./09-troubleshooting.md#1-症状速查表)。
...
只有一条路：**`text()` 写进去的东西**，累积成本次运行的返回值，由客户端打印出来。

其他两个看起来像输出的 primitive 在 PoA 下都不管用：

| primitive | 它做什么 | PoA 下 |
| --- | --- | --- |
| `text(v)` | 往本次运行的返回内容里追加一条 | ✅ **唯一可靠的** |
