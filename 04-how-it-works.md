...
模型只拿到一个叫 `exec` 的工具。所有真正干活的工具被折叠进这个工具的说明文本里，渲染成 TypeScript 声明，由 JS 调用：
...
>
> 工具声明之所以被折进 `exec` 的 description，也只是因为模型手上只剩 `exec` 这一个工具，别的工具没地方声明了。工具契约本身没有被重新设计。
...
codex 原本的 code mode 里，那段 JavaScript 是模型写的，它只是模型一只更灵巧的手。

PoA 做的事是开一个口子，让模型以外的东西也能提交 JS。
...
**② 模型侧有的能力，程序侧不一定有。** 因为这个入口只开了"提交"这一件事。最典型的是续跑：模型可以让一个中途让出的 cell 接着跑，程序没有对应的手段。详见[边界与限制](./08-limits.md)。
...
调用发生时，沙箱当场造一个未完成的 Promise 返回给 JS，同时把这次调用作为一个事件交给宿主。宿主侧走的是正常的工具分发路径，与模型直接调这个工具完全同一条路，同样的审批、同样的记录。执行完之后：
...
写程序时按「全部派出去 → 一次 join → 统一收口」来组织，别设计成流水线。
...
`yield_control()` 的问题在于：它交出前半段之后程序还在跑，但没有配套的手段去接后半段。模型有（它可以用 `wait` 工具续跑），程序没有。
