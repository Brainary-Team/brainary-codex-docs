...
> [!NOTE]
> 这个字段以前叫 `source`，直接收源码字符串。2026-08 改成收包，为的是让程序能**自带 MCP server**——包展开、起 server、跑 entry 全在服务端做，所以任何前端（CLI、TUI、SDK）跑一个包都只是把同一份字节递过去。详见[核心概念 §9](./03-concepts.md#9-poa-包自带能力的那条路)。
...
**③ 输出只有 `text()` 一条路。** 只有写进 `text()` 的内容才会回到终端；中间过程的一切都留在沙箱里。
