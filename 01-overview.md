...
> 实现了包入口的前端只有 `codex exec --poa`，TUI 与 SDK 都没有。详见核心概念 §9。
...
另有几件事这条通道做不到，动手之前先知道：

| 想做的事 | 当前的行为 |
| --- | --- |
| 中途问用户问题 | 内置的 `request_user_input` 有三道门，每一道单独就足以挡死：要开实验开关、曝光度是 `DirectModelOnly`（code mode 根本拿不到）、且只有根线程能用。另外 `codex exec --poa` 这条路把审批策略钉成 `never` 且覆盖不掉，问人的请求一律在毫秒级自动 decline |
| 按完成顺序边收边处理（流式）、早停、等待期间再派新的 | 程序侧的等待调用串行，详见工作原理。有一条绕法：把重活放进一个标了只读的 MCP 工具，那类工具是并行安全的，而且可以随包分发 |
| 中途取消一个跑飞的程序 | 没有任何 RPC 能中止正在跑的 cell，跑飞了只能杀进程 |
| 超时之后接着跑 | yield 之后没有配套手段接回来 |
| 严格的结构化输出保证 | 没有内核级的 schema 校验，只能在 prompt 里要求 + 防御性解析 |
