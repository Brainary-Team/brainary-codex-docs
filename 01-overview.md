...
>
> 至于工具声明为什么跑到 `exec` 的说明文本里去——那只是因为**模型手上只剩 `exec` 这一个工具**，别的工具再没有地方可以声明。
> 工具本身的参数和行为**一个字都没变**，换的只是谁来调它们。
...
`package` 是一个 base64 编码的 `.poa` 包——**一个 zip，根上放 `manifest.toml`，里面是那段 JS 和它要带的东西**。

**这个接口只接受包，不再接受裸源码。** 手上只有一段 JS、没有任何要随行的能力时，仍须先把它封成一个不申报任何能力的最小包再提交——`run_workflow.py` 正是这么做的。协议层没有第二个只收源码的入口。
...
> **设计上如此，但目前实现了包入口的前端只有 `codex exec --poa`**——TUI 与 SDK 都还没有。详见[核心概念 §9](./03-concepts.md#9-poa-包自带能力的那条路)。
