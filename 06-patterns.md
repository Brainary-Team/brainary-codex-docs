...
`demos/06`-`10` 不提供可复用的编排形状——它们是**能力样本**：不给出程序的组织方式，而是回答"这套配置下实际有哪些工具、每个工具真实返回什么形状"。

| demo | 值得抄的是 |
| --- | --- |
| `06_capability_matrix.js` | **每次换模型 / provider / profile 之后先跑它。** 它报告哪些工具在、哪些不在、不在的那些卡在哪道门上 |
| `07_exec_session.js` | 怎么用 `exec_command` + `write_stdin` 维持一个会话；以及**怎么用计时把"真并行"和"串行"区分开** |
| `08_file_and_image.js` | 一次写操作要过的**三道互相独立的门**：exec 策略、沙箱模式、审批模式 |
