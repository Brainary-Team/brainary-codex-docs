...
> [!NOTE]
> **首行那个 `// @exec:` 必须真的在第 1 行。** 它是这次运行的配置，两个字段的默认值都只有 10000（10 秒 / 10000 token）。派了 agent 的程序必须调大 `yield_time_ms`，否则程序还没等到第一个 agent 就被截断。详见[编写指南 §1](./05-writing.md#1-首行-pragma)。
